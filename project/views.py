...
def report(request, project_id):
    if request.method == 'POST':
        try:
            if len(list(
                    Report_project.objects.filter(user_id=request.session['logged_in_user'], project_id=project_id))):
    ...
    if request.method == 'POST':
        try:
            ...
            if float(project.current_money) / project.target >= 0.25:
                messages.error(request, 'Your cant cancel project if the donations are less than 25% of the target ',
                               extra_tags='report')
                return redirect(f"/project/{project_id}")
            else:
                project.delete()
